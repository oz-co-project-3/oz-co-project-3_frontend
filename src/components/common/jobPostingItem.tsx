...
export default function JobPostingItem({ post, detailPagePath }: JobPostingItemProps) {
  return (
    <div className='relative flex flex-col gap-4 rounded-md border px-2 py-2 transition-all duration-200 hover:bg-zinc-100 sm:justify-between sm:py-6'>
      <Link className='block' href={`${detailPagePath}/${post.id}`}>
        <div className='flex flex-col justify-between py-2 max-sm:gap-3 sm:flex-row'>
          {/* 근무지 */}
          <div className='flex w-full flex-col items-center justify-center px-4 pl-2 text-center sm:h-[80px] sm:w-[150px]'>
            <span>{post.location}</span>
          </div>
          {/* 모집제목/기업명 */}
          <div className='flex w-full flex-col justify-center px-4 text-center sm:h-[80px] sm:w-[300px]'>
          ...
          {/* 근무형태 */}
          <div className='flex w-full flex-col items-center justify-center px-4 sm:h-[80px] sm:w-[150px]'>
            <span className='w-15 rounded-sm bg-gray-200 text-center'>{post.employment_type}</span>
          </div>
          {/* 마감일 */}
          <div className='flex w-full flex-col items-center justify-center px-4 sm:h-[80px] sm:w-[280px]'>
